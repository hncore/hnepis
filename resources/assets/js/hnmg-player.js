...
window.hnsetcolor = function () {
    ...
    $.each(colorVars, function (cssVar, colorKey) {
        const colorValue = player_epis?.hncolor?.[colorKey] || colorKey;
        if (colorValue) {
...
window.Load_Player = function () {
    return {
        ...
        async haunPlayer(episodeSlug, serverId, postId, nonce, subsvId, customVar) {
            ...
            try {
                const response = await axios.post(player_epis.player_url, data, {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Content-Type': 'application/x-www-form-urlencoded',
                    }
                ...
                if (response.data.success) {
                    ...
                    if (await checkAdBlock()) {
                        $("#hn-player-loader").empty().html(player_epis.adblock_msg);
                    } else {
                        $("#hn-player-loader").empty();
                        localStorage.removeItem("reSizePlayerObject");
                        var resumeId = hnresponse.resumeId;
                        var hnsources = hnresponse.encryptedSources;
                        if(resumeId && hnsources){
                            var tracks = hnresponse.tracks;
                            this.HNJwplayer(tracks, hnsources, resumeId);
                        } else {
        ...
        HNJwplayer(tracks, hnsources, resumeId) {
            var decryptedSources = this.decryptEncData(hnsources);
            ...
            if (typeof playerInstance !== 'undefined') {
                const config = {
                    ...
                    height: "100%",
                    hlshtml: true,
                    autostart: player_epis.autoplay,
                    fullscreen: true,
                    playbackRateControls: true,
                    displayPlaybackLabel: true,
                    aspectratio: "16:9",
                };
                if (player_epis.player_logo) {
                    config.logo = {
                ...
                if (player_epis.floating) {
                    config.floating = {
                        dismissible: true,
                    };
                }
                if (player_epis.autopause) {
                    config.autoPause = {
                        viewability: true,
                        pauseAds: true,
                    };
                ...
                if (player_epis.jwplayer_showad) {
                    config.ads = player_epis.jw_adcode;
                }
                ...
                if (player_epis.adblock) {
                    playerInstance.on('ready', async function () {
                        if(await checkAdBlock()){
                            playerInstance.pause();
                            jQuery("#hn-player-loader").show().html(player_epis.adblock_msg);
                        }
