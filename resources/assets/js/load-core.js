...
if (hncore.disable_debug == 1) {
    ...
    if (!is_Mobile()) {
        ...
        window.addEventListener('hncore', (e) => {
            if (e.detail.open) {
                window.location = hncore.db_redirect_url;
            }
        });
        ...
        if (window.location.href.startsWith('view-source:')) {
            window.location.href = hncore.db_redirect_url;
        }
...
if (hncore.is_single && hncore.select_comments) {
    $('body').append('<div id="fb-root"></div>');
    window.fbAsyncInit = function () {
        FB.init({ 
            appId: hncore.fb_apps_id, 
            cookie: true, 
            xfbml: true, 
            version: 'v3.0' 
        });
...
window.hncorecolor = function () {
    ...
    $.each(colorVars, function (cssVar, colorKey) {
        const colorValue = hncore?.corecolor?.[colorKey] || colorKey;
        if (colorValue) {
