...
window.HNEpis = function() {
	return {
		...
		async hnimporterone(postId, importType) {
			const list_link = document.getElementById('import-multi-importone').value.trim().split(/\r?\n/).filter(Boolean);
			const server = document.getElementById("serverListsOne").value;
			const server_name = document.getElementById('serverListsOne').selectedOptions[0].text;
			const episodeSlug = document.getElementById('listepisode-one').value;

			if (importType !== 'listepisode' && !episodeSlug) {
				alert(___('notice_epi_to_import'));
				return;
			}
			if (!list_link.length || list_link[0] === '') {
				alert(___('please_link_list'));
				return;
			}

			document.getElementById('statusone').style.display = 'block';
			document.getElementById('statusone').innerHTML = ___('please_wait');
			...
			try {
				...
				if (response.data.success) {
					await this.HNListEpByServerID(postId, server);
					document.getElementById('statusone').innerHTML = ___('import_successfuly');
					setTimeout(function() {
						window.location.reload();
					}, 1000);
				} else {
					document.getElementById('statusone').innerHTML = ___('error');
				}
			} catch (error) {
				document.getElementById('statusone').innerHTML = ___('error');
		...
		hnimportertwo(postId) {
			...
			if (!server_name) {
				alert(___('please_server_name'));
				return;
			} else if (!list_link.length) {
				alert(___('please_form_notice'));
				return;
			} else {
				$('#status').show().html(___('please_wait'));
				...
				.then(response => {
					if (response.data.success) {
						$('#statustwo').show();
						$('#statustwo .hnnotice').html(___('import_successfuly'));
						$('#importtwo').html(___('import_episode'));
						setTimeout(function() {
							window.location.reload();
						}, 1000)
					} else {
						$('#statustwo .hnnotice').html(___('error'));
					}
				})
				.catch(error => {
					$('#statustwo .hnnotice').html(___('error'));
		...
		hnaddserver(postId) {
			...
			for (const link of list_link) {
				...
				if (data.length !== 3) {
					alert(___('error_structure') + ': ' + link);
					return;
				}
			}
			$('#status').show().html(___('please_wait'));
			...
			.then(response => {
				if (response.data.success) {
					$('#hnstatus').show().html(___('import_successfuly'));
			...
			if (confirm('Are you sure you want to delete the selected posts?')) {
				...
				.then(response => {
					if (response.data.success) {
						...
						console.error('Failed to delete selected posts:', response.data.message);
					}
				})
				.catch(error => console.error('Error:', error));
			}
