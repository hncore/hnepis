...
window.HNEpis = function() {
	return {
		...
		hnaddserver(postId) {
			...
			showToast(___('please_wait'), 'info');
			const formData = new FormData();
			formData.append('action', 'hnajax_addnewserver');
			formData.append('post_id', postId);
			formData.append('server', server);
			formData.append('server_name', server_name);
			list_link.forEach((link, index) => {
				const data = link.split('|');
				const ep_name = data[0].trim();
				const ep_link = data[1].trim();
				const ep_type = data[2].trim();
				const ep_slug = ep_name.toLowerCase().replace(/[^a-z0-9]/g, '-');
				formData.append(`haunmovies_ep_name[${server}][${index}]`, ep_name);
				formData.append(`haunmovies_ep_slug[${server}][${index}]`, ep_slug);
				formData.append(`haunmovies_ep_link[${server}][${index}]`, ep_link);
				formData.append(`haunmovies_ep_type[${server}][${index}]`, ep_type);
			});
			axios.post(ajax_url, formData, {
				headers: {
					'Content-Type': 'application/x-www-form-urlencoded',
...
jQuery(document).ready(function($) {
                ...
                <div id="${modalId}" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 hidden">
                    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                        <div class="mt-3 text-center">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">${___(title)}</h3>
                            <div class="mt-2 px-7 py-3">
                                <p class="text-sm text-gray-500">${___(message)}</p>
                            </div>
                            <div class="items-center px-4 py-3">
                                <button class="bg-blue-500 text-white px-4 py-2 rounded-md text-sm mr-2 hover:bg-blue-600 focus:outline-none">${___(buttonText)}</button>
                                <button class="close-modal bg-gray-300 px-4 py-2 rounded-md text-sm hover:bg-gray-400 focus:outline-none">${___('Cancel')}</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    if ($('.hncore_create_page, .hncore_create_menu, .hncore_create_widgets, .hncore_import_themes').length > 0) {
    ...
    function handleAjaxAction(buttonClass, modalId, messageId, action, successText) {
        $(`${modalId} .${buttonClass}`).on('click', function() {
            $(messageId).html(`
                <div class="bg-blue-100 text-blue-700 px-4 py-2 rounded mt-2 flex items-center space-x-2">
            ...
            $.ajax({
                ...
                success: function(response) {
                    $(messageId).html(`
                        <div class="bg-green-100 text-green-700 px-4 py-2 rounded mt-2">
                            <p><strong>${___(successText)}</strong> ${___('havefun')}</p>
                        </div>
                    `);
                },
                error: function(xhr, status, error) {
                    $(messageId).html(`
                        <div class="bg-red-100 text-red-700 px-4 py-2 rounded mt-2">
                            <p>${___('Error occurred during the process.')}</p>
                        </div>
                    `);
                }
            });
    ...
    $('.hncore_create_page').on('click', function() {
        $('#create_page_modal').removeClass('hidden').hide().fadeIn(300);
    });
    $('.hncore_create_menu').on('click', function() {
        $('#create_menu_modal').removeClass('hidden').hide().fadeIn(300);
    });
    $('.hncore_create_widgets').on('click', function() {
        $('#create_widgets_modal').removeClass('hidden').hide().fadeIn(300);
    });
    $('.hncore_import_themes').on('click', function() {
        $('#import_themes_modal').removeClass('hidden').hide().fadeIn(300);
    });
    ...
    // Gọi hàm xử lý AJAX cho từng hành động
    handleAjaxAction('bg-blue-500', '#create_page_modal', '#create_page_message', 'hncore_create_page', 'page');
    handleAjaxAction('bg-blue-500', '#create_menu_modal', '#create_menu_message', 'hncore_create_menu', 'menu');
    handleAjaxAction('bg-blue-500', '#create_widgets_modal', '#create_widgets_message', 'hncore_create_widgets', 'widgets');
    handleAjaxAction('bg-blue-500', '#import_themes_modal', '#import_themes_message', 'hncore_import_theme_demo', 'theme');
});
